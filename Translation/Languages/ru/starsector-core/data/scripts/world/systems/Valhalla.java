...
public class Valhalla {
	...
	public void generate(SectorAPI sector) {
		...
		system.addAsteroidBelt(star, 100, 3150, 256, 150, 250, Terrain.ASTEROID_BELT, "The Valkyries");
			...
			// Valkyrie jump
			JumpPointAPI jumpPoint = Global.getFactory().createJumpPoint("valkyrie_jump", " Valkyrie Точка прыжка");
...
//		initStationCargo(mimir_station);
	...
	// orbited by station, relay, and asteroid field each 1/3rd orbit off from one another.
		...
		// Ragnar Complex
			SectorEntityToken ragnar_station = system.addCustomEntity("ragnar_complex", "Ragnar Комплекс", "station_side02", "hegemony");
		...
		// Ragnar Relay
			SectorEntityToken relay = system.addCustomEntity("ragnar_relay", // unique id
					 "Ретрансляционная станция связи Ragnar", // name - if null, defaultName from custom_entities.json will be used
		...
		JumpPointAPI jumpPoint_ragnar = Global.getFactory().createJumpPoint("ragnar_jump", "Ragnar Точка прыжка");
